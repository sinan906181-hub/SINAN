...
export const AdminMessagesTab: React.FC<AdminMessagesTabProps> = ({
  ...
  const handleOpenEmailReply = (inquiry: ContactInquiry) => {
    soundManager.playClick();
    const subject = encodeURIComponent(`Re: ${inquiry.topic} - Response from Sinan K`);
    const body = encodeURIComponent(
      `Hi ${inquiry.name},\n\nThank you for reaching out regarding "${inquiry.topic}".\n\n${quickReplyText || 'I reviewed your note and would love to connect further.'}\n\nBest regards,\nSinan K\nCreative Developer & Designer\nhttps://chat.google.com/`
    );
        ...
        <div className="flex items-center gap-3.5">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">
              Direct notes submitted by clients & visitors through your portfolio contact form.
            </p>
          </div>
        </div>
        ...
        <div className="lg:col-span-7">
          {selectedInquiry ? (
            <div className="p-6 rounded-3xl bg-[#0c101a]/90 border border-white/10 shadow-xl space-y-6">
              ...
              {/* Quick Reply & Dispatch Tools */}
              <div className="pt-4 border-t border-white/10 space-y-3">
                <span className="text-[11px] font-mono-code uppercase tracking-wider text-slate-400 block">
                  Compose Quick Response
                </span>
                <textarea
                  rows={3}
                  value={quickReplyText}
                  onChange={(e) => setQuickReplyText(e.target.value)}
                  placeholder={`Draft a quick response note to ${selectedInquiry.name}...`}
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                  ...
                  <button
                    onClick={() => handleOpenEmailReply(selectedInquiry)}
                    className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-amber-500 to-amber-400 hover:from-amber-400 hover:to-amber-300 text-slate-950 font-bold text-xs font-display shadow-lg shadow-amber-500/20 transition-all flex items-center gap-2 cursor-pointer"
                  >
                    <Send className="w-3.5 h-3.5" />
                    <span>Send Reply via Email ({selectedInquiry.email})</span>
                  </button>
                </div>
              </div>
            </div>
